    ...
    def test_article_gsi2_reverse_sort_key_calculation(self):
        ...
        for score, expected_key in test_cases:
            ...
            assert gsi2_sk == expected_key, (
                f"Score {score} should generate {expected_key}, got {gsi2_sk}"
            )
...
class TestKeywordModel:
    ...
    def test_keyword_creation_with_valid_data(self):
        """有効なデータでキーワードを作成"""
        keyword = Keyword(text="Python", weight=1.5, is_active=True)
...
class TestLinkIndexModel:
    ...
    def test_link_index_creation(self):
        ...
        link_index = LinkIndex.create_from_article(
            "https://example.com/article", "article-123"
        )
    ...
    def test_link_index_hash_generation(self):
        ...
        # 末尾のスラッシュは正規化される
        hash3 = LinkIndex.generate_hash_from_url(
    ...
    def test_link_index_duplicate_detection(self):
        ...
        assert link_index.is_duplicate_of(
            "https://example.com/article/"
        )  # 末尾スラッシュ
        assert link_index.is_duplicate_of(
            "HTTPS://EXAMPLE.COM/ARTICLE"
        ...
        # 異なるURLは重複として検出されない
        assert not link_index.is_duplicate_of(
    ...
    def test_link_index_validation_errors(self):
        ...
        # 空の記事IDはエラー
        with pytest.raises(ValidationError):
            LinkIndex(link="https://example.com/article", article_id="")
