...
def valid_feed_strategy(draw):
    ...
    return Feed(url=url, title=title, folder=folder, is_active=is_active)
...
def valid_keyword_strategy(draw):
    ...
    return Keyword(text=text, weight=weight, is_active=is_active)
...
class TestArticleModelProperties:
    ...
    def test_reverse_sort_key_ordering(self, score1: float, score2: float):
        ...
        if score1_int > score2_int:
            assert key1 < key2, (
                f"score1={score1} > score2={score2} なので key1={key1} < key2={key2} であるべき"
            )
            ...
            assert key1 == key2, (
                f"score1={score1} == score2={score2} (整数化後) なので key1={key1} == key2={key2} であるべき"
            )
...
class TestLinkIndexProperties:
    ...
    @given(
        url=valid_url_strategy(), article_id=st.uuids().map(lambda x: x.hex)
    )
    @settings(max_examples=50)
    ...
    @settings(max_examples=50)
    def test_link_index_duplicate_detection(
        self, url1: str, url2: str, article_id: str
    ):
        """
        リンクの重複検出の正確性テスト
        """
...
class TestModelValidationProperties:
    ...
    def test_non_positive_keyword_weight_validation(self, weight: float):
        ...
        """
        with pytest.raises(ValidationError):
            Keyword(text="test keyword", weight=weight)
