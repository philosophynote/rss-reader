...
- [x] 2. AWS CDKインフラストラクチャの実装
  - [x] 2.1 DynamoDBスタックを実装
    - テーブル定義とGSIの設定
    - **GSI3（created_at用）の追加（効率的な記事削除のため）**
    - **GSI4（既読記事削除用）の追加**
    - **GSI5（フィード別記事クエリ用、カスケード削除のため）の追加**
    - **TTL設定の追加（自動削除機能）**
    - _要件: 11.1, 11.2, 12.1, 12.2_

  - [x] 2.2 Lambda関数スタックを実装
    - [x] 2.2.1 Lambda関数の基本定義を実装
      - Docker Image Functionの定義
      - メモリサイズとタイムアウトの設定
      - アーキテクチャの設定
      - _要件: 8.2, 12.2_

    - [x] 2.2.2 環境変数とIAM権限の設定を実装
      - DynamoDB、Bedrockへのアクセス権限
      - 基本環境変数の設定（テーブル名、リージョン等）
      - _要件: 8.3, 12.3_

    - [x] 2.2.3 認証関連の環境変数設定を実装
      - API Key環境変数の設定
      - CORS許可オリジンの設定
      - _要件: 9.1, 9.4_

    - [x] 2.2.4 Lambda関数URLの設定を実装
      - AWS IAM認証の設定
      - CORS設定の強化（特定オリジンのみ許可）
      - 認証情報を含むリクエストの許可設定
      - _要件: 9.1, 9.4, 12.3_

  - [x] 2.3 EventBridgeスタックを実装
    - フィード取得ジョブ（1時間ごと）
    - 記事削除ジョブ（1日1回）
    - _要件: 8.4, 12.2_

  - [x] 2.4 フロントエンドスタックを実装
    - S3バケットとCloudFrontディストリビューションの設定
    - _要件: 12.2_

  - [x] 2.5 CDKデプロイスクリプトを作成
    - 環境別デプロイの設定
    - _要件: 12.4, 12.5_

- [x] 3. DynamoDBクライアントとデータモデルの実装
  - [x] 3.1 データモデルクラスを実装
    - Feed、Article、Keyword、ImportanceReason、LinkIndexのPydanticモデルを作成
    - PK/SK/GSI生成メソッドを実装（GSI1～GSI5対応）
    - TTL設定メソッドを実装
    - **GSI2SKのゼロパディング処理を実装（importance_scoreを文字列ソート可能な形式に変換）**
    - **逆順ソートキー生成メソッドを実装（重要度スコア用：(1000000 - score * 1000000)をゼロパディング）**
    - デザインドキュメントのエンティティ設計に準拠
    - _要件: 8.5_

  - [x] 3.2 DynamoDBクライアントクラスを実装
    - put_item、get_item、query、delete_item、batch_write_itemメソッドを実装
    - GSI1～GSI5を使用したクエリメソッドを実装
    - 時系列順・重要度順ソート用のクエリメソッドを実装
    - **効率的な削除クエリメソッドを実装（GSI3、GSI4使用、Scan操作を回避）**
    - **カスケード削除用のクエリメソッドを実装（GSI5使用）**
    - _要件: 8.5_

  - [x] 3.3 データモデルのプロパティテストを作成
    - **プロパティ1: フィード登録の永続化**
    - **検証: 要件 1.1**
    - PK/SK生成の一意性テスト
    - GSI生成の正確性テスト

  - [x] 3.4 データモデルのユニットテストを作成
  ...
  - [x] 3.5 DynamoDBクライアントのユニットテストを作成
    - 基本的なCRUD操作のテスト
    - GSIクエリのテスト
    - エラーハンドリングのテスト
    - バッチ操作のテスト
...
- [x] 4. フィード管理機能の実装
  - [x] 4.1 FeedServiceクラスを実装
    - add_feed、get_feeds、delete_feed、update_feedメソッドを実装
    - **delete_feed時のカスケード削除（GSI5を使用して関連記事を効率的に検索・削除）**
    - **関連記事の重要度理由も含めた完全なカスケード削除**
    - **バッチ削除処理による効率的な削除**
    - _要件: 1.1, 1.2, 1.3, 1.4_

  - [x] 4.2 フィード管理のプロパティテストを作成
  ...
  - [x] 4.3 フィード管理のユニットテストを作成
    - 正常系のテスト
    - エッジケースのテスト
    - _要件: 1.1, 1.2, 1.3, 1.4_

- [x] 5. チェックポイント - すべてのテストが通過することを確認
  - すべてのテストが通過することを確認し、質問があればユーザーに尋ねる

- [x] 6. AWS Bedrockセマンティック検索の実装
...
- [x] 7.1 FeedFetcherServiceクラスを実装
  ...
  - [x] 7.3 フィード取得のユニットテストを作成
    - RSS 2.0とAtomフィードのパーステスト
    - エラーハンドリングのテスト
    - ImportanceScoreServiceとの統合テスト
    - _要件: 2.2, 10.1_

- [x] 8. チェックポイント - すべてのテストが通過することを確認
  - すべてのテストが通過することを確認し、質問があればユーザーに尋ねる
...
- [x] 10. キーワード管理機能の実装
  ...
  - [x] 10.3 キーワード管理のユニットテストを作成
    - デフォルト重みのテスト
    - 再計算機能のテスト
    - _要件: 6.3, 7.5_

- [x] 11. チェックポイント - すべてのテストが通過することを確認
  - すべてのテストが通過することを確認し、質問があればユーザーに尋ねる

- [x] 12. 記事削除機能の実装
  - [x] 12.1 CleanupServiceクラスを実装
    - cleanup_old_articles、delete_articles_by_age、delete_read_articlesメソッドを実装
    - **GSI3を使用したクエリベースの古い記事削除（Scanを使用しない効率的な削除）**
    - **GSI4を使用したクエリベースの既読記事削除**
    - **TTLによる自動削除との連携（ハイブリッドアプローチ）**
    - **バッチ削除処理による効率的な削除**
    - _要件: 11.1, 11.2, 11.3, 11.4_

  - [x] 12.2 記事削除のプロパティテストを作成
    - **プロパティ24: 古い記事の削除**
    - **検証: 要件 11.1**
    - **プロパティ25: 既読記事の削除**
    - **検証: 要件 11.2**
  ...
  - [x] 12.3 記事削除のユニットテストを作成
    - 削除条件のテスト
    - カスケード削除のテスト
    - _要件: 11.1, 11.2, 11.4_

- [x] 13. FastAPI エンドポイントと認証の実装
  - [x] 13.1 API Key認証ミドルウェアを実装
    - [x] 13.1.1 HTTPBearer認証スキームの設定
      - FastAPIセキュリティスキームの定義
      - verify_api_key関数の実装
      - _要件: 9.1_

    - [x] 13.1.2 セキュリティミドルウェアの実装
      - セキュリティヘッダーミドルウェアの追加
      - 機密情報マスキングログフィルターの実装
      - _要件: 9.2, 9.5_

    - [x] 13.1.3 レート制限ミドルウェアの実装
      - クライアント別レート制限の実装
      - レート制限超過時のエラーレスポンス
      - _要件: 9.3_

  - [x] 13.2 CORS設定とセキュリティ強化を実装
    - 特定オリジンのみ許可するCORS設定
    - 機密情報マスキングログフィルターの実装
    - 入力検証とサニタイゼーションの追加
    - _要件: 9.4, 9.5_

  - [x] 13.3 フィード管理エンドポイントを実装
    - POST /api/feeds、GET /api/feeds、DELETE /api/feeds/{feed_id}、PUT /api/feeds/{feed_id}
    - API Key認証を全エンドポイントに適用
    - _要件: 1.1, 1.2, 1.4, 9.1_

  - [x] 13.4 記事管理エンドポイントを実装
  ...
  - [x] 13.6 ジョブ実行エンドポイントを実装
    - POST /api/jobs/fetch-feeds、POST /api/jobs/cleanup-articles
    - API Key認証を全エンドポイントに適用
    - _要件: 2.1, 11.3, 9.1_

  - [x] 13.7 認証機能のプロパティテストを作成
    - **プロパティ31: API Key認証の検証**
    - **検証: 要件 9.1**
    - **プロパティ32: 認証失敗時のエラーレスポンス**
    - **検証: 要件 9.2**

  - [x] 13.8 認証機能のユニットテストを作成
    - 有効なAPI Keyでのアクセステスト
    - 無効なAPI Keyでの認証失敗テスト
    - レート制限のテスト
    - _要件: 9.1, 9.2, 9.3_

  - [x] 13.9 APIエンドポイントの統合テストを作成
    - 認証ありでのエンドツーエンドテスト
    - エラーレスポンスのテスト
    - セキュリティヘッダーの検証
    - _要件: 1.1, 3.1, 6.1, 9.1, 9.4_

- [x] 14. チェックポイント - すべてのテストが通過することを確認
  - すべてのテストが通過することを確認し、質問があればユーザーに尋ねる

- [x] 15. Lambda関数とEventBridgeの設定
  - [x] 15.1 Lambda関数のエントリーポイントを作成
    - FastAPIアプリケーションを起動するmain.pyを作成
    - _要件: 8.2, 8.3_

  - [x] 15.2 Dockerfileを完成させる
...
- [x] 16. フロントエンドの実装
  ...
  - [x] 16.5 記事一覧コンポーネントを実装
    ...
    - [x] 16.5.4 記事状態表示と操作ボタンの実装
      - 未読/既読状態、重要度スコアの表示
      - 既読/未読切り替えボタン
      - 保存/解除ボタン
      - _要件: 3.3, 4.1, 4.2, 5.1, 5.2_

  - [x] 16.6 記事詳細コンポーネントを実装
  ...
  - [x] 16.7 キーワード管理コンポーネントを実装
    - KeywordList、KeywordFormコンポーネントを作成
    - Chakra UIを使用したフォームデザイン
    - _要件: 6.1, 6.2, 6.4, 6.5, 6.6_

  - [x] 16.8 フロントエンド認証のユニットテストを作成
    - APIクライアントの認証テスト
    - エラーバウンダリーのテスト
    - 認証失敗時のコンポーネント動作テスト
    - _要件: 8.7, 8.8, 9.1, 9.2_

  - [x] 16.9 フロントエンドのユニットテストを作成
    - コンポーネントのレンダリングテスト
    - ユーザーインタラクションのテスト
    - Chakra UIコンポーネントのテスト
    - _要件: 8.7, 8.8_

- [x] 17. チェックポイント - すべてのテストが通過することを確認
  - すべてのテストが通過することを確認し、質問があればユーザーに尋ねる

- [x] 18. CI/CDパイプラインの実装
  - [x] 18.1 GitHub Actionsワークフローファイルを作成
    - [x] 18.1.1 CI基本ワークフロー（ci.yml）を作成
      - プッシュ・プルリクエストトリガーの設定
      - 基本的なジョブ構造の定義
      - _要件: 13.1, 13.2_

    - [x] 18.1.2 バックエンドテストジョブを実装
      - Python環境のセットアップ
      - リンティング、型チェック、テストの自動実行
      - カバレッジレポートの生成
      - _要件: 13.4, 13.5_

    - [x] 18.1.3 フロントエンドテストジョブを実装
      - Node.js環境のセットアップ
      - リンティング、型チェック、テストの自動実行
      - ビルドテストの実行
      - _要件: 13.4_

    - [x] 18.1.4 インフラストラクチャテストジョブを実装
      - CDK synthとdiffの実行
      - インフラストラクチャの検証
      - _要件: 13.4_

  - [x] 18.2 バックエンドデプロイメントワークフローを作成
    - deploy-backend.ymlを作成
    - Docker イメージのビルドとECRへのプッシュ
    - Lambda関数の更新
    - _要件: 13.6, 13.7_

  - [x] 18.3 フロントエンドデプロイメントワークフローを作成
    - deploy-frontend.ymlを作成
    - ビルドとS3へのデプロイ
    - CloudFrontキャッシュの無効化
    - _要件: 13.6, 13.7_

  - [x] 18.4 インフラストラクチャデプロイメントワークフローを作成
    - deploy-infra.ymlを作成
    - CDKを使用したインフラストラクチャの自動デプロイ
    - _要件: 13.6, 13.7_

  - [x] 18.5 CI/CDパイプラインのプロパティテストを作成
  ...
  - [x] 18.6 GitHub Secretsとセキュリティ設定
    - AWS認証情報の設定
  ...
  - [x] 18.7 セキュリティ強化されたデプロイメント設定
    - Parameter Storeでの機密情報暗号化
    - デプロイ時のAPI Key環境変数設定
    - 認証テストの追加
    - _要件: 9.1, 9.6, 13.7_

  - [x] 18.8 統合テストとヘルスチェックの実装
    - デプロイメント後のヘルスチェック
    - 認証ありでの統合テストの自動実行
    - API Key認証の検証テスト
    - _要件: 9.1, 13.8, 13.9_
